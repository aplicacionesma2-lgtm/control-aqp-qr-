...
Flujo:
  1. Sube el Excel del pedido (hojas "data" y "Receta") en la barra lateral.
  2. En la pestaña "Etiquetas" genera e imprime las etiquetas QR por producto
     (el valor del QR es el mismo CÓDIGO de SAP).
  3. En la pestaña "Escanear", el operario escanea la caja, digita la
     cantidad de cajas empacadas y confirma. La app calcula unidades
     (cajas x FACTOR) y las acumula contra el REQUERIMIENTO del pedido.
  4. "Avance del pedido" muestra el detalle y estado por producto.
  5. "Historial" permite auditar / corregir escaneos.
  6. "Exportar" genera un Excel con el resumen y el historial.

Persistencia: Google Sheets (vía gspread), mismo patrón que la app
`entregas`. Se usan 3 hojas dentro de UN spreadsheet: "Pedido",
"Catalogo" y "Registros" — se crean solas la primera vez que la app
corre. Requiere credenciales de una cuenta de servicio de Google Cloud
en `st.secrets` (ver README para la configuración paso a paso).
"""
...
PEDIDO_HEADERS = ["codigo", "descripcion", "umi", "factor", "requerimiento", "fecha_pedido"]
CATALOGO_HEADERS = ["codigo", "producto", "factor", "umr"]
REGISTROS_HEADERS = ["id", "codigo", "producto", "cantidad_cajas", "factor", "unidades", "umi", "operario", "timestamp"]


def config_faltante() -> list:
    faltan = []
    if "gcp_service_account" not in st.secrets:
        faltan.append("`[gcp_service_account]` (credenciales de la cuenta de servicio)")
...
def _rows_seguras(df: pd.DataFrame, columnas: list, numericas: set) -> list:
    """Convierte un DataFrame a listas de valores para Sheets, preservando
    tipos numéricos como float/str explícitos (RAW) para que Google Sheets
    no intente reinterpretar números como fechas u otros formatos."""
    filas = []
...
def procesar_excel(file):
    """Lee y valida el excel subido. Devuelve (data_df, receta_df, error)."""
    try:
        xl = pd.ExcelFile(file)
    except Exception as e:
        return None, None, f"No se pudo leer el archivo: {e}"

    if "data" not in xl.sheet_names or "Receta" not in xl.sheet_names:
        return None, None, "El Excel debe tener las hojas 'data' y 'Receta'."
    ...
    if not req_data.issubset(set(data_df.columns)):
        ...
        return None, None, f"A la hoja 'data' le faltan columnas: {faltan}"
    if not req_receta.issubset(set(receta_df.columns)):
        faltan = req_receta - set(receta_df.columns)
        return None, None, f"A la hoja 'Receta' le faltan columnas: {faltan}"

    return data_df, receta_df, None


def guardar_pedido(data_df: pd.DataFrame, receta_df: pd.DataFrame):
    d = data_df[["CÓDIGO", "DESCRIPCIÓN", "UMI", "FACTOR", "REQUERIMIENTO", "FECHA PEDIDO"]].copy()
    d.columns = PEDIDO_HEADERS
    d["codigo"] = d["codigo"].astype(str).str.strip()
    d["fecha_pedido"] = pd.to_datetime(d["fecha_pedido"], errors="coerce").dt.strftime("%Y-%m-%d")

    c = receta_df.drop_duplicates(subset="CÓDIGO")[["CÓDIGO", "PRODUCTO", "FACTOR", "UMR"]].copy()
    ...
    ws_pedido.append_rows(_rows_seguras(d, PEDIDO_HEADERS, {"factor", "requerimiento"}), value_input_option="RAW")

    ws_catalogo = get_ws("Catalogo", tuple(CATALOGO_HEADERS))
    ws_catalogo.clear()
    ws_catalogo.append_row(CATALOGO_HEADERS, value_input_option="RAW")
    ws_catalogo.append_rows(_rows_seguras(c, CATALOGO_HEADERS, {"factor"}), value_input_option="RAW")
...
def registrar_escaneo(codigo, producto, cajas, factor, unidades, umi, operario):
    ws = get_ws("Registros", tuple(REGISTROS_HEADERS))
    fila = [
        uuid.uuid4().hex[:8], str(codigo), str(producto), float(cajas), float(factor),
        float(unidades), str(umi), str(operario), datetime.now().strftime("%Y-%m-%d %H:%M:%S"),
    ]
...
def generar_hoja_etiquetas(productos, columnas: int = 3) -> Image.Image:
    ...
    for idx, (codigo, nombre) in enumerate(productos):
        ...
        draw.rectangle([x0 + 5, y0 + 5, x0 + label_w - 5, y0 + label_h - 5], outline=C_PRIMARY, width=2)
        qr_img = generar_qr(codigo, size=280)
        sheet.paste(qr_img, (x0 + (label_w - 280) // 2, y0 + 15))
        draw.text((x0 + label_w // 2, y0 + 300), codigo, font=font, fill="black", anchor="mm")
        nombre_corto = nombre if len(nombre) <= 40 else nombre[:37] + "..."
        draw.text((x0 + label_w // 2, y0 + 330), nombre_corto, font=font_small, fill="black", anchor="mm")
...
if faltantes:
    ...
    st.error(
        "Faltan credenciales de Google Sheets en `secrets`: " + ", ".join(faltantes)
        + ". Revisa el README (sección 'Configurar Google Sheets') para configurarlas."
    )
    st.stop()
...
with st.sidebar:
    ...
    operario = st.text_input("👤 Nombre del operario", value=st.session_state.get("operario", ""))
    st.session_state["operario"] = operario

    st.markdown("---")
    st.markdown("### 📁 Pedido del día")
    archivo = st.file_uploader("Excel con hojas 'data' y 'Receta'", type=["xlsx"])
    if archivo is not None:
        firma_archivo = f"{archivo.name}-{archivo.size}"
        if firma_archivo != st.session_state.get("ultimo_archivo_cargado"):
            data_df, receta_df, error = procesar_excel(archivo)
            if error:
                st.error(error)
            else:
                guardar_pedido(data_df, receta_df)
                st.cache_data.clear()
                st.session_state["ultimo_archivo_cargado"] = firma_archivo
                st.success(f"Pedido cargado: {len(data_df)} productos.")
                st.rerun()

    pedido_df, catalogo_df = cargar_pedido()
    if not pedido_df.empty:
        fechas = ", ".join(sorted(pedido_df["fecha_pedido"].dropna().unique()))
        st.caption(f"📅 {fechas}  \n{len(pedido_df)} productos en pedido · {len(catalogo_df)} en catálogo")

    st.markdown("---")
    with st.expander("⚠️ Reiniciar historial del día"):
        st.caption("Borra todos los escaneos registrados. No afecta el pedido ni el catálogo.")
...
if pedido_df.empty or catalogo_df.empty:
    st.title("📦 Control de Empaque QR")
    st.info("👈 Sube el Excel del pedido del día (hojas **data** y **Receta**) en la barra lateral para comenzar.")
    st.stop()
...
tab_scan, tab_avance, tab_hist, tab_labels, tab_export = st.tabs(
    ["📷 Escanear", "📊 Avance del pedido", "🕒 Historial", "🏷️ Etiquetas", "📤 Exportar"]
)
...
with tab_scan:
    if not operario:
        st.warning("Ingresa tu nombre en la barra lateral antes de registrar escaneos.")

    st.markdown("#### Escanea el QR de la caja")
    codigo_leido = qrcode_scanner(key="scanner")

    col_m1, col_m2 = st.columns([3, 1])
    codigo_manual = col_m1.text_input("O ingresa el código manualmente", key="manual_input")
    buscar_manual = col_m2.button("🔍 Buscar", use_container_width=True)

    if codigo_leido and codigo_leido.strip() != st.session_state.get("ultimo_escaneo_procesado"):
    ...
    if codigo_actual:
        ...
        if cat_row.empty:
            st.error(f"❌ Código **{codigo_actual}** no está en el catálogo (hoja Receta).")
        else:
            producto = cat_row.iloc[0]["producto"]
            ped_row = pedido_df[pedido_df["codigo"] == codigo_actual]

            if ped_row.empty:
                st.warning(f"⚠️ **{producto}** no tiene pedido registrado hoy. Se puede registrar solo para trazabilidad.")
                factor, umi, requerimiento = cat_row.iloc[0]["factor"], cat_row.iloc[0]["umr"], None
            else:
            ...
            if requerimiento:
                ...
                ya_entregado = float(fila_av["unidades_entregadas"].iloc[0]) if not fila_av.empty else 0.0
                pct_actual = ya_entregado / requerimiento * 100 if requerimiento else 0
                st.progress(
                    min(pct_actual / 100, 1.0),
                    text=f"{fmt_num(ya_entregado)} / {fmt_num(requerimiento)} {umi} entregadas ({pct_actual:.1f}%)",
                )

            cajas = st.number_input(
                "Cantidad de cajas escaneadas / empacadas", min_value=0.0, step=1.0, value=1.0,
            ...
            if cb1.button("✅ Registrar entrega", type="primary", use_container_width=True):
                registrar_escaneo(
                    codigo_actual, producto, cajas, float(factor), unidades_calc, umi,
                    operario or "Sin nombre",
                )
                st.session_state["ultimo_escaneo_procesado"] = codigo_actual
                st.session_state["codigo_actual"] = None
                st.success(f"Registrado: {fmt_num(cajas)} cajas de {producto} ({fmt_num(unidades_calc)} {umi}).")
                st.rerun()
...
with tab_avance:
    ...
    filtro_estado = col_f2.multiselect(
        "Estado", ["⛔ Sin iniciar", "🟡 Parcial", "✅ Completo"],
    ...
    if busqueda:
        mask = (
            df_show["codigo"].str.contains(busqueda, case=False, na=False)
            | df_show["descripcion"].str.contains(busqueda, case=False, na=False)
        )
        df_show = df_show[mask]
    df_show = df_show[df_show["estado"].isin(filtro_estado)]

    colores_estado = {"⛔ Sin iniciar": C_BAD, "🟡 Parcial": C_WARN, "✅ Completo": C_OK}
    ...
    for _, row in df_show.iterrows():
        ...
        c2.progress(
            ...
            text=f"{fmt_num(row['unidades_entregadas'])}/{fmt_num(row['requerimiento'])} {row['umi']} — {row['pct_avance']:.1f}%",
        )
        c3.markdown(
            f"<span class='badge' style='background-color:{colores_estado[row['estado']]}'>{row['estado'].split()[0]}</span>",
            unsafe_allow_html=True,
        )
        ...
        data=[
            go.Pie(
                ...
                values=[
                    ...
                    int(((avance_df["pct_avance"] > 0) & (avance_df["pct_avance"] < 100)).sum()),
    ...
    fig.update_layout(title="Distribución de productos por estado", height=380, showlegend=True)
...
with tab_hist:
    if registros_df.empty:
        ...
        operarios = ["Todos"] + sorted(registros_df["operario"].dropna().unique().tolist())
        filtro_op = st.selectbox("Filtrar por operario", operarios)
        df_hist = registros_df if filtro_op == "Todos" else registros_df[registros_df["operario"] == filtro_op]

        st.dataframe(
            df_hist[["id", "timestamp", "operario", "codigo", "producto", "cantidad_cajas", "unidades", "umi"]],
            use_container_width=True, hide_index=True,
        )

        st.markdown("#### Eliminar / corregir un registro")
        opciones_borrado = {
            f"#{r.id} — {r.timestamp} — {r.producto} ({fmt_num(r.cantidad_cajas)} cajas, {r.operario})": r.id
            for r in df_hist.itertuples()
        }
        if opciones_borrado:
            sel = st.selectbox("Selecciona el registro a eliminar", list(opciones_borrado.keys()))
...
# ----------------------------------------------------------------
with tab_labels:
    st.markdown("Genera etiquetas QR para pegar en las cajas. El valor del QR es el mismo **código SAP** del producto.")
    opciones = (catalogo_df["codigo"] + " — " + catalogo_df["producto"]).tolist()
    seleccion = st.multiselect("Selecciona productos (vacío = todos los del pedido de hoy)", opciones)
    columnas_hoja = st.slider("Columnas por hoja", min_value=2, max_value=5, value=3)
    ...
    if st.button("🏷️ Generar hoja de etiquetas"):
        ...
        productos = (
            catalogo_df[catalogo_df["codigo"].isin(codigos_sel)][["codigo", "producto"]].values.tolist()
        )
        if not productos:
            ...
            st.image(hoja, caption=f"{len(productos)} etiqueta(s) generada(s)", use_container_width=True)
            st.download_button(
                "⬇️ Descargar hoja de etiquetas (PNG, para imprimir)",
                data=buf.getvalue(), file_name="etiquetas_qr.png", mime="image/png",
            )
...
# ----------------------------------------------------------------
with tab_export:
    st.markdown("Exporta el detalle de avance y el historial completo de escaneos a un Excel.")
    resumen_export = avance_df.rename(
        columns={
            "codigo": "Código", "descripcion": "Descripción", "umi": "U. Medida",
            "factor": "Factor", "requerimiento": "Requerimiento",
            "unidades_entregadas": "Entregado", "cajas_entregadas": "Cajas entregadas",
            "pct_avance": "% Avance", "estado": "Estado", "fecha_pedido": "Fecha pedido",
        }
    )
